...
# Save the CPU cache sizes to the specified file for each node
pdsh -w $PBS_NODEFILE "echo 'L1 cache size: $L1_cache_size' >> $1_\$(hostname).txt; echo 'L2 cache size: $L2_cache_size' >> $1_\$(hostname).txt; echo 'L3 cache size: $L3_cache_size' >> $1_\$(hostname).txt"
